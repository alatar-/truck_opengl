...
vertex_2d<float>Obstacle::get_model_min_point() {
	return model->get_min_point();
}

vertex_2d<float>Obstacle::get_model_max_point() {
	return model->get_max_point();
}

vector <vertex_2d<float> >  Obstacle::get_model_vertices() {
    // printf("\n%p> Vehicle::get_body_vertices\n", this);

    dimensions.clear();
    vertex_2d<float>pos_min = get_model_min_point();
    // printf("vertex: ( %f , %f ) \n", pos_min.x, pos_min.y);
    vertex_2d<float>pos_max = get_model_max_point();
