...
// 		return I;
		...
		vertex_3d v = get_scaling(anim_time);
...
vertex_3d animation_t::get_scaling (float anim_time) {
	if (rotations.size() == 0) {
		return vertex_3d(1.0f, 1.0f, 1.0f);
	} else if (rotations.size() == 1) {
		vertex_3d sc = scalings[0].scale;
		return sc;
	} else {
		unsigned i
			,	j;
		float factor = key_bisection(scalings, anim_time, full_time, i, j);
		vertex_3d sc = vertex_3d::lerp(scalings[i].scale, scalings[j].scale, factor);
		return sc;
	}
}

vertex_3d animation_t::get_translation (float anim_time) {
	if (translations.size() == 0) {
		return vertex_3d(0.0f, 0.0f, 0.0f);
	} else if (translations.size() == 1) {
		vertex_3d tr = translations[0].trans;
		return tr;
	} else {
		unsigned i
			,	j;
		float factor = key_bisection(translations, anim_time, full_time, i, j);
		vertex_3d tr = vertex_3d::lerp(translations[i].trans, translations[j].trans, factor);
		return tr;
	}
}
