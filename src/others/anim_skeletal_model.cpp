...
anim_skeletal_Model::anim_skeletal_Model() : anim_Model() {
}

anim_skeletal_Model::~anim_skeletal_Model() {
	clear();
}

void anim_skeletal_Model::clear() {
	Model::clear();
	skeleton.clear();
}

bool anim_skeletal_Model::load (string filename, float in_fps, unsigned in_frames) {
	fps = in_fps;
	return Model::load(filename);
}

bool anim_skeletal_Model::parse_scene (const aiScene *pScene, string &model_file) {
	if (Model::parse_scene(pScene, model_file)) {
...
void anim_skeletal_Model::init_bones (const aiScene *pScene) {
// 	printf("anim_skeletal_Model::init_bones\n");
...
// 		printf("anim_skeletal_Model::init_bones> #%u: map_bones(%p, %p)\n", i, pScene->mMeshes[i], meshes[i]);
		map_bones(pScene->mMeshes[i], meshes[i], bone_map);
	}
// 	printf("anim_skeletal_Model::init_bones> bones_mapped, size=%lu\n", bone_map.size());
...
// 	printf("anim_skeletal_Model::init_bones> structure set\n");
}

void anim_skeletal_Model::map_bones (aiMesh *paiMesh, Mesh *mesh, map <string, bone_t*> &bone_map) {
// 	printf("anim_skeletal_Model::map_bones(%p, %p)\n", paiMesh, mesh);
	for (unsigned i = 0, ilen = paiMesh->mNumBones; i < ilen; ++i) {
// 	printf("anim_skeletal_Model::map_bones(%p, %p)> in loop #%u\n", paiMesh, mesh, i);
		const aiBone *paiBone = paiMesh->mBones[i];
// 		printf("anim_skeletal_Model::map_bones(%p, %p)> in loop #%u, bone: %p\n", paiMesh, mesh, i, paiBone);
		string bone_name(paiBone->mName.data);
		bone_t *the_bone;
// 		printf("anim_skeletal_Model::map_bones> pre_bad_printf\n");
// 		printf("anim_skeletal_Model::map_bones> #%u < %u, name: %s\n", i, ilen, bone_name.c_str());
// 		printf("anim_skeletal_Model::map_bones> post_bad_printf\n");
...
// 			printf("anim_skeletal_Model::map_bones> weight #%u < %u\n", j, jlen);
...
void anim_skeletal_Model::set_structure_and_animations (const aiScene *pScene, aiNode *node, bone_t *parent, map <string, bone_t*> &bone_map) {
	string node_name(node->mName.data);
	const aiNodeAnim *pNodeAnim = find_node_anim(pScene->mAnimations[0], node_name);
	bone_t *the_bone;
	
// 	printf("anim_skeletal_Model::set_structure_and_animations(%p, %p), name: %s\n", node, parent, node_name.c_str());
	
	if (bone_map.find(node_name) == bone_map.end()) {
		the_bone = new bone_t();
		skeleton.add_bone(the_bone);
// 		printf("anim_skeletal_Model::set_structure_and_animations(%p, %p): NEW BONE: %p, name: %s\n", node, parent, the_bone, node_name.c_str());
	} else {
		the_bone = bone_map[node_name];
	}
	the_bone->set_parent(parent);
// 	printf("anim_skeletal_Model::set_structure_and_animations> parent(%p) set to %p\n", parent, the_bone->get_parent());
	
	if (pNodeAnim) {
		the_bone->set_anim_full_time(full_time);
		
// 		printf("anim_skeletal_Model::set_animations> pos keys: %u\n", pNodeAnim->mNumPositionKeys);
// 		printf("anim_skeletal_Model::set_animations> scl keys: %u\n", pNodeAnim->mNumScalingKeys);
// 		printf("anim_skeletal_Model::set_animations> rot keys: %u\n", pNodeAnim->mNumRotationKeys);
...
// 		printf("anim_skeletal_Model::set_structure_and_animations(%p, %p): NO ANIMATION on bone(%p), name: %s\n", node, parent, the_bone, node_name.c_str());
	}
...
// 	printf("anim_skeletal_Model::set_structure_and_animations> children: %u\n", node->mNumChildren);
	for (unsigned i = 0, len = node->mNumChildren; i < len; ++i) {
		set_structure_and_animations(pScene, node->mChildren[i], the_bone, bone_map);
	}
}

const aiNodeAnim* anim_skeletal_Model::find_node_anim (const aiAnimation* pAnimation, const string NodeName) {
// 	printf("anim_skeletal_Model::find_node_anim(%s)\n", NodeName.c_str());
...
void anim_skeletal_Model::recalc_vertices() {
// 	printf("anim_skeletal_Model::recalc_vertices\n");
...
void anim_skeletal_Model::animate (float anim_time) {
	if (full_time) {
