...
player_t::player_t (world_t *in_world, float in_pos_x, float in_pos_y, float in_ang_h, float in_height, float in_speed, float in_size) : pos(in_pos_x, in_pos_y) {
	world = in_world;
...
void player_t::move (direct_t keys_h, direct_t keys_v) {
...
glm::mat4 player_t::get_view_matrix() {
	return glm::lookAt(
		glm::vec3(pos.x, height, pos.y)
	,	glm::vec3(pos.x + 10 * sin(ang_h), 10 * sin(ang_v), pos.y + 10 * cos(ang_h))
